...
public class PurchaseController implements Initializable{
	...
	@FXML
	private TableColumn<ProductDetails, ComboBox> unitCol;
	@FXML
	private TableColumn<ProductDetails, TextField> unitQuantityCol;
	@FXML
	private TableColumn<ProductDetails, String> quantityCol;
	@FXML
	private TableColumn<ProductDetails, TextField> unitPriceCol;
	@FXML
		...
		if(addValidationCheck()) {
			...
			for(rowCount = 0;rowCount < listProductDetails.size();rowCount++) {
				...
				if(tempPd.getItemId().equals(itemId)) {
						...
						listProductDetails.add(rowCount,new ProductDetails(itemId, getCmbItemName(), unit, unitQuantity, quantity, unitPrice, unitPrice, totalAmount, discount, netAmount,this));
			...
			if(rowCount == listProductDetails.size()) {
					...
					listProductDetails.add(new ProductDetails(itemId, getCmbItemName(), unit, unitQuantity, quantity, unitPrice, unitPrice, totalAmount, discount, netAmount,this));
					clearTxtAfterItemAdd();
			...
			if(!isInvoiceIdExist(getTxtInvoiceNO())) {
				if(saveValidaionCheck()) {
					if(confirmationCheck("Save")) {
						...
						String purchaseLedger = "6";
						String purchaseDiscountLedger = "10";
						String purchaseVatLedger = "78";
						String d_l_id;
						String c_l_id;
						String cash="0";
						String card="0";
						...
						if(Double.valueOf(getTxtTotalAmount())>0) {
							...
							sql ="insert into tbaccftransection(transectionid,voucherno,type,d_l_id, c_l_id,amount,description, date, entrytime, createby) "
									+ "values('"+transectionAutoId()+"','"+invoiceAutoNo+"','"+type+"','"+d_l_id+"','"+c_l_id+"','"+getTxtTotalAmount()+"','Purchase','"+dbDateFormate.format(date.getValue())+"',CURRENT_TIMESTAMP,'"+SessionBeam.getUserId()+"')";
						...
						if(vatAmount>0) {
							...
							d_l_id = purchaseVatLedger;
							c_l_id = supplierledgerId;
							String accTransection2="insert into tbaccftransection(transectionid,voucherno,type,d_l_id, c_l_id,amount,description, date, entrytime, createby) values('"+transectionAutoId()+"','"+invoiceAutoNo+"','"+type+"','"+d_l_id+"','"+c_l_id+"','"+vatAmount+"','Purchase Vat','"+dbDateFormate.format(date.getValue())+"',CURRENT_TIMESTAMP,'"+SessionBeam.getUserId()+"')";
						...
						if (Double.parseDouble(getTxtTotalDiscount())>0) {
							...
							c_l_id = purchaseDiscountLedger;
							String accTransection2="insert into tbaccftransection(transectionid,voucherno,type,d_l_id, c_l_id,amount,description, date, entrytime, createby) values('"+transectionAutoId()+"','"+invoiceAutoNo+"','"+type+"','"+d_l_id+"','"+c_l_id+"','"+txtTotalDiscount.getText()+"','Purchase Discount','"+dbDateFormate.format(date.getValue())+"',CURRENT_TIMESTAMP,'"+SessionBeam.getUserId()+"')";
							databaseHandler.execAction(accTransection2);
						...
						if (Double.parseDouble(txtPaidAmount.getText()) >= 0) {
							...
							if(getCmbPayementMethod().equals("Cash")) {
								...
								sql="insert into tbaccftransection(transectionid,voucherno,type,d_l_id, c_l_id,amount,description, date, entrytime, createby) values('"+transectionAutoId()+"','"+invoiceAutoNo+"','"+type+"','"+d_l_id+"','"+c_l_id+"','"+txtPaidAmount.getText()+"','Cash Purchase','"+dbDateFormate.format(date.getValue())+"',CURRENT_TIMESTAMP,'"+SessionBeam.getUserId()+"')";
								databaseHandler.execAction(sql);
								...
								d_l_id = supplierledgerId;
								c_l_id = paymentLedgerId;
								sql="insert into tbaccftransection(transectionid,voucherno,type,d_l_id, c_l_id,amount,description, date, entrytime, createby) values('"+transectionAutoId()+"','"+invoiceAutoNo+"','"+type+"','"+d_l_id+"','"+c_l_id+"','"+txtPaidAmount.getText()+"','Card Purchase','"+dbDateFormate.format(date.getValue())+"',CURRENT_TIMESTAMP,'"+SessionBeam.getUserId()+"')";
								databaseHandler.execAction(sql);
				...
				if(editValidaionCheck()) {
					...
					if(confirmationCheck("Edit")) {
						...
						for(int i=0;i<cashTId.length;i++) {
							sql = "update tbaccftransection set d_l_id = '"+supplierledgerId+"' where transectionid = '"+cashTId[i]+"' and type = '"+type+"'";

							databaseHandler.execAction(sql);
						}
						for(int i=0;i<cardTId.length;i++) {
							sql = "update tbaccftransection set d_l_id = '"+supplierledgerId+"' where transectionid = '"+cardTId[i]+"' and type = '"+type+"'";	
							databaseHandler.execAction(sql);
						}

						//this Step For Update Total Amount
						if(Double.valueOf(getTxtTotalAmount())>0) {
							sql = "select * from tbaccftransection where voucherno='"+invoiceAutoNo+"' and d_l_id='"+purchaseLedger+"' and type='"+type+"'";
							rs = databaseHandler.execQuery(sql);
							if(rs.next()) {
								int tranId= rs.getInt("transectionid");
								commonTransection = String.valueOf(tranId);
								sql = "update tbaccftransection set amount = '"+getTxtTotalAmount()+"',c_l_id='"+supplierledgerId+"' where transectionid='"+tranId+"' and type='"+type+"' and d_l_id = '"+purchaseLedger+"' and voucherNo='"+invoiceAutoNo+"'";
								databaseHandler.execAction(sql);
								...
								d_l_id = purchaseLedger;
								c_l_id = supplierledgerId;
								sql ="insert into tbaccftransection(transectionid,voucherno,type,d_l_id, c_l_id,amount,description, date, entrytime, createby) values('"+transectionAutoId()+"','"+invoiceAutoNo+"','"+type+"','"+d_l_id+"','"+c_l_id+"','"+txtTotalAmount.getText()+"','Purchase','"+dbDateFormate.format(date.getValue())+"',CURRENT_TIMESTAMP,'"+SessionBeam.getUserId()+"')";
								databaseHandler.execAction(sql);
						...
						if (Double.parseDouble(getTxtTotalDiscount())>0) {
							sql = "select * from tbaccftransection where voucherno='"+invoiceAutoNo+"' and c_l_id='"+purchaseDiscountLedger+"' and type='"+type+"'";
							rs = databaseHandler.execQuery(sql);
							if(rs.next()) {
								int tranId= rs.getInt("transectionid");
								discountTransection = String.valueOf(tranId);
								sql = "update tbaccftransection set amount = '"+getTxtTotalDiscount()+"' ,d_l_id='"+supplierledgerId+"' where transectionid='"+tranId+"' and type='"+type+"' and  c_l_id = '"+purchaseDiscountLedger+"' and voucherNo='"+invoiceAutoNo+"'";
								databaseHandler.execAction(sql);
								...
								d_l_id = supplierledgerId;
								c_l_id = purchaseDiscountLedger;
								sql="insert into tbaccftransection(transectionid,voucherno,type,d_l_id, c_l_id,amount,description, date, entrytime, createby) values('"+transectionAutoId()+"','"+invoiceAutoNo+"','"+type+"','"+d_l_id+"','"+c_l_id+"','"+getTxtTotalDiscount()+"','Purchase Discount','"+dbDateFormate.format(date.getValue())+"',CURRENT_TIMESTAMP,'"+SessionBeam.getUserId()+"')";
								databaseHandler.execAction(sql);
						...
						if (vatAmount>0) {
							sql = "select * from tbaccftransection where voucherno='"+invoiceAutoNo+"' and d_l_id='"+purchaseVatLedger+"' and type='"+type+"'";
							rs = databaseHandler.execQuery(sql);
							if(rs.next()) {
								int tranId= rs.getInt("transectionid");
								vatTransection = String.valueOf(tranId);
								sql = "update tbaccftransection set amount = '"+vatAmount+"' ,c_l_id='"+supplierledgerId+"' where transectionid='"+tranId+"' and type='"+type+"' and  d_l_id = '"+purchaseVatLedger+"' and voucherNo='"+invoiceAutoNo+"'";
								databaseHandler.execAction(sql);
								...
								d_l_id = purchaseVatLedger;
								c_l_id = supplierledgerId;
								sql="insert into tbaccftransection(transectionid,voucherno,type,d_l_id, c_l_id,amount,description, date, entrytime, createby) values('"+transectionAutoId()+"','"+invoiceAutoNo+"','"+type+"','"+d_l_id+"','"+c_l_id+"','"+vatAmount+"','Purchase Vat','"+dbDateFormate.format(date.getValue())+"',CURRENT_TIMESTAMP,'"+SessionBeam.getUserId()+"')";
								databaseHandler.execAction(sql);
		...
		try {
			if(!getCmbFind().isEmpty()) {
				if(isInvoiceIdExist(getCmbFind())) {
					...
					rs = databaseHandler.execQuery(sql);
					while(rs.next()) {
						listProductDetails.add(new ProductDetails(rs.getString("itemId"),rs.getString("itemName"), rs.getString("unit"), rs.getDouble("UnitQty"), rs.getDouble("qty"), rs.getDouble("unitPrice"), rs.getDouble("PurchasePrice"), rs.getDouble("totalamount"), rs.getDouble("discount"), rs.getDouble("netAmount"),this));
					}
	...
	private void btnDuePayAction(ActionEvent event) {
		try {		
			String payType = "Purchase Due Payment";

			String purchaseLedgerId= "6";
	...
	private void invoiceListTableLoad(String supplierName) {
		try {
			if(supplierName.equals("")) {
				...
				sql = "select invoice,SellerCustomerName,totalAmount from tbInvoice where type = '"+type+"' and SellerCustomerName = '"+supplierName+"' order by Invoice desc";
			}
	...
	public void categoryNameLoad() {
		try {
			sql = "select categoryName from tbCategory where id != 1";
			ResultSet rs = databaseHandler.execQuery(sql);
	...
	private void setCmpAction() {
		...
		cmbCategory.valueProperty().addListener(new ChangeListener<String>() {
			@Override public void changed(ObservableValue ov, String oldValue, String newValue) {
				if(newValue != null) {
					itemListLoadByCategory(newValue);
				}
			}    
		});
	...
	private void itemListLoadByCategory(String categoryName){
		try{
			CategoryInfo category = LoadedInfo.getCategoryInfo(categoryName);
			if(category != null){
				sql = "select * from tbItem where CategoryId = '"+category.getCategoryId()+"'";
				ResultSet rs = databaseHandler.execQuery(sql);	
				int i=0;
				String itemId;
				while(rs.next()){
					itemId = rs.getString("id");
					for(i = 0 ; i < listProductDetails.size() ; i++){
						if(listProductDetails.get(i).getItemId().equals(itemId)){	
							break;
						}
					}
					if(i == listProductDetails.size()){
						listProductDetails.add(new ProductDetails(itemId, rs.getString("projectedItemName"), rs.getString("Unit")+" (1.0)", 0, 0, rs.getDouble("purchasePrice"), rs.getDouble("purchasePrice"), 0, 0, 0,this));
					}
				}
				tableItemDetails.setItems(listProductDetails);	
			}else{
				new Notification(Pos.TOP_CENTER, "Warning graphic", "Warning!","Please Select Valid Category Name..");
	...
	private void unitLoadByProductName() {
		try {
	...
	private void setCmpData() {
		...
		unitCol.setCellValueFactory(new PropertyValueFactory("cmbUnit"));
		unitQuantityCol.setCellValueFactory(new PropertyValueFactory("txtUnitQuantity"));
		quantityCol.setCellValueFactory(new PropertyValueFactory("qunatityWithFree"));
		unitPriceCol.setCellValueFactory(new PropertyValueFactory("txtUnitPrice"));
	...
	private void addCmp() {
		...
		cmbCategory.setPrefWidth(435);
		cmbCategory.setPrefHeight(28);

		vBoxCategoryName.getChildren().remove(1);
		vBoxCategoryName.getChildren().add(1,cmbCategory);
		...
		tableItemDetails.getColumns().get(4).setVisible(false);
		tableItemDetails.getColumns().get(6).setVisible(false);
		tableItemDetails.getColumns().get(8).setVisible(false);
	}
	...
	public static class ProductDetails{
		...
		private TextField txtUnitQuantity;
		...
		public ProductDetails(String itemId,String itemName,String unit,double unitQuantity,double quantity,double unitPrice,double price,double totalAmount,double discount,double netAmount,PurchaseController purchaseController) {
			...
			this.txtUnitQuantity = new TextField();
			this.txtUnitPrice = new TextField();
			this.cmbUnit = new ComboBox<>();
			
			
			this.purchaseController = purchaseController;
		
			this.txtUnitPrice.focusedProperty().addListener((observable,oldValue,newValue) ->{
				Platform.runLater(() -> {
					if (txtUnitPrice.isFocused()  && !txtUnitPrice.getText().isEmpty()) {
			...
			this.txtUnitPrice.setTextFormatter(NumberField.getDoubleFormate());
			this.txtUnitPrice.focusedProperty().addListener((ov, oldV, newV) -> {
				if (!newV ) { // focus lost
					if(this.txtUnitPrice.getText().isEmpty()) {
						setUnitPrice(unitPrice);
					}
					calculation();
					this.purchaseController.netAmountCount();
				}
			});
			
			
			this.txtUnitQuantity.focusedProperty().addListener((observable,oldValue,newValue) ->{
				Platform.runLater(() -> {
					if (txtUnitQuantity.isFocused()  && !txtUnitQuantity.getText().isEmpty()) {
						txtUnitQuantity.selectAll();
					}
				});
			});
			
			this.txtUnitQuantity.setTextFormatter(NumberField.getDoubleFormate());
			this.txtUnitQuantity.focusedProperty().addListener((ov, oldV, newV) -> {
				if (!newV ) { // focus lost
					if(this.txtUnitQuantity.getText().isEmpty()) {
						setUnitPrice(unitPrice);
					}
					calculation();
					this.purchaseController.netAmountCount();
			...
			this.txtUnitQuantity.setText(String.valueOf(unitQuantity));
			this.txtUnitPrice.setText(String.valueOf(unitPrice));
			this.cmbUnit.setItems(LoadedInfo.getUnitList(itemId));
			
			this.cmbUnit.getSelectionModel().select(unit);;
		}
		...
		private void calculation() {
			// TODO Auto-generated method stub
			String unit = getCmbUnit().getSelectionModel().getSelectedItem().toString();
			
			unit = unit.substring(unit.indexOf('(')+1,unit.indexOf(')'));
			Double n= Double.valueOf(unit);
			Double unitQty = getUnitQuantity();
			Double unitPrice = getUnitPrice();
			
			setPrice(unitPrice * n);
			setQuantity(unitQty * n);
			setTotalAmount(unitQty * unitPrice);
			setNetAmount(getTotalAmount() - getDiscount());
			
			this.purchaseController.tableItemDetails.refresh();
		}
		...
		public String getUnit() {
			return cmbUnit.getSelectionModel().getSelectedItem().toString();
		}
		public void setUnit(String unit) {
			this.cmbUnit.getSelectionModel().select(unit);
		}
		public double getUnitQuantity() {
			if(txtUnitQuantity.getText().trim().isEmpty()){
				return 0;
			}else{
				return Double.valueOf(txtUnitQuantity.getText().trim());
			}
			
		}
		public void setUnitQuantity(double unitQuantity) {
			txtUnitQuantity.setText(String.valueOf(unitQuantity));
		}
		
		public TextField getTxtUnitQuantity() {
			
			return txtUnitQuantity;
		}
		...
		public double getUnitPrice() {
			if(txtUnitPrice.getText().isEmpty()) return 0.0;
			return Double.valueOf(txtUnitPrice.getText().trim());
		}
