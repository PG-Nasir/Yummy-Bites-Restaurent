...
public class KitchenIssueController implements Initializable{
	...
	private final int finishedGoodsType=1;
	private final int rawMaterialsType=2;
	...
	private void btnPrintAction(ActionEvent event) {
		try {
			if(isInvoiceIdExist(getTxtInvoiceNO())) {
				...
				String report = "src/resource/reports/Invoices/KitchenIssueInvoice.jrxml";
	...
	public void productNameLoad() {
		try {
			sql = "select projectedItemName from tbItem where isActive = '1' and type = '"+rawMaterialsType+"' order by projectedItemName";
			ResultSet rs = databaseHandler.execQuery(sql);
