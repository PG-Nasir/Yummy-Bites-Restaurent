...
import ui.accounts.CashPaymentController.InvoiceList;
import ui.accounts.CashPaymentController.VoucherDetails;
import shareClasses.FxComboBox;

public  class CashRecivedController implements Initializable{
	...
	@FXML
	TextField txtTotalReciveableAmount;
	...
	String salesType = "3";
	String servicingSalesType = "5";
	String servicingType = "6";
	...
	private void btnAddAction(ActionEvent event) {
		if(!isVoucherNoExist()) {
			if(ledgerAddValidationCheck()) {
				if(doubleDebitLedgerAndDateCheck()) {
					if(!doubleCreditLedgerCheck()) {
						...
						new Notification(Pos.TOP_CENTER, "Warning graphic", "Warning!","You Can't Entry Double Credit Ledger .");
						cmbCreditLedger.requestFocus();
					}
					
				}else {
					new Notification(Pos.TOP_CENTER, "Warning graphic", "Warning!","You Can't Entry Multiple Debit Ledger And Date In a Credit Voucher.\nPlease Enter Same Debit Ledger and Date...");
					cmbCreditLedger.requestFocus();
				}
			}
		}else {
	...
	private void saveAction(ActionEvent event) {
		try {
			if(!isVoucherNoExist()) {
				if(list.size()>0) {
					if(listInvoiceList.size()>0 && getCheckInvoiceRecived()) {
						if(isTotalPayableAmountExist()) {
							if(confrimationCheck("Save")) {
								double availableAmount = getTotalRecivedAmount();
								String invoiceNo;
								...
								String payType = "Due Recived By Cash Recived";
								String transectionId;
								String d_l_id;
								String c_l_id;
								
								sql = "insert into tbpaymentHistory (invoiceNo,cash, card,due,depID,type,date,paymentType,flag,entryTime,entryBy) values ( '" + getTxtVoucherNo() + "', '" + availableAmount + "','0','0','" + 1 + "','" + voucherType + "','" + new SimpleDateFormat("yyyy-MM-dd").format(new Date()) + "','Cash','Total Sales Due Recived',CURRENT_TIMESTAMP, '" + SessionBeam.getUserId() + "' )";
								databaseHandler.execAction(sql);
								...
								sql = "insert into tbAccftransection (transectionid,voucherNo,type,Status,unitId,depId,d_l_id,c_l_id,"
										...
										+ "'1','"+getTxtNarration()+"','"+getTxtPaidTo()+"','"+getDate()+"','Sales Due Cash Recived','','',CURRENT_TIMESTAMP,'"+SessionBeam.getUserId()+"');";
								...
								for(int i=0;i<listInvoiceList.size() && availableAmount>0 ;i++) {
									...
									if(tempInvoice.idCheckSelect()) {
										...
										sql = "insert into tbpaymentHistory (invoiceNo,cash, card,due,depID,type,date,paymentType,flag,entryTime,entryBy) values ( '" + invoiceNo + "', '" + invoiceDuePay + "','0','" + invoiceDueAmount + "','" + 1 + "','" + salesType + "','" + new SimpleDateFormat("yyyy-MM-dd").format(new Date()) + "','Cash','" + payType + "',CURRENT_TIMESTAMP, '" + SessionBeam.getUserId() + "' )";
										...
										*/
										...
										sql="update tbInvoice set paid="+invoicePaidAmount+",cash="+invoiceCash+",card="+invoiceCard+",due="+invoiceDueAmount+", cashid='"+invoiceCardId+","+transectionId+"' where invoice='"+invoiceNo+"' and type='"+salesType+"'";
	...
	private void printAction(ActionEvent event) {
		...
		try {
			if(isVoucherNoExist()) {
				...
				String report="src/resource/reports/Invoices/CashRecivedtVoucher.jrxml";
	...
	private double getTotalRecivedAmount() {
	...
	private boolean confrimationCheck(String name) {
		...
		return AlertMaker.showConfirmationDialog("Confirmation", "Are You Sure to "+name+" this Credit Voucher?");
	}

	private boolean doubleDebitLedgerAndDateCheck() {
		// TODO Auto-generated method stub
		if(list.size()>0) {

			VoucherDetails tempVoucher = list.get(0);
			if(tempVoucher.getDate().equals(getDate()) && tempVoucher.getDebitLedger().equals(getCmbDebitLedger())) {
				return true;
			}else{
				return false;
			}
	...
	private boolean doubleCreditLedgerCheck() {
		// TODO Auto-generated method stub
		if(list.size()>0) {

			VoucherDetails tempVoucher = list.get(0);
			for(int i = 0;i<list.size();i++) {
				if(list.get(i).getCreditLedger().equals(getCmbCreditLedger())) {
					return true;
				}
			}
	...
	private boolean isTotalPayableAmountExist() {
		...
		if(tempTotal<=Double.valueOf(getTxtTotalReciveableAmount())) {
			return true;
		}	
		return false;
	...
	private void loadDueInvoiceByCustomerLedger(String ledgerName) {
		// TODO Auto-generated method stub
		try {
			listInvoiceList.clear();
			tableInvoiceList.setItems(listInvoiceList);

			sql = "select l.ledgerTitle,c.ID,c.CustomerName from tbAccfledger l\r\n" + 
					"join tbCustomer c\r\n" + 
					"on l.ledgerId = c.ledgerId where ledgerTitle = '"+ledgerName+"'";
			ResultSet rs = databaseHandler.execQuery(sql);
			...
			if(rs.next()) {
				...
				while(rs2.next()) {
					InvoiceList tempInvoice = new InvoiceList(rs2.getString("invoice"), rs2.getDouble("totalamount"), rs2.getDouble("netAmount"), rs2.getDouble("paid"), rs2.getDouble("due"), rs2.getDouble("cash"), rs2.getDouble("card"), rs2.getString("cashid"), rs2.getString("cardId"), "Sales",false);
					...
					tempInvoice.getCheckSelect().setOnAction(e->{
						...
						setTxtTotalReciveableAmount(total);
					});
			...
			if(listInvoiceList.size()>0) {
				setCheckInvoiceRecived(true);
				list.clear();
			}else {
				setCheckInvoiceRecived(false);
			}

			table.setItems(list);
			setTxtTotalReciveableAmount("0");
	...
	private void setCmpFocusAction() {
		...
		txtTotalReciveableAmount.focusedProperty().addListener((observable,oldValue,newValue) ->{
			selectTextIfFocused(txtTotalReciveableAmount);
		});
	...
	private void cmpAction() {
		...
		cmbCreditLedger.valueProperty().addListener(new ChangeListener<String>() {
			@Override 
			public void changed(ObservableValue ov, String oldValue, String newValue) {
				if(newValue != null) {
					loadDueInvoiceByCustomerLedger(newValue);
	...
	public static class InvoiceList{
		...
		public InvoiceList(String invoiceNo,double totalAmount,double netAmount,double paidAmount,double dueAmount,double cashAmount,double cardAmount,String cashId,String cardId,String type,boolean isSelect) {
		...
		public void setType(String type) {
			this.cardId = new SimpleStringProperty(type);
		}
	...
	public String getTxtTotalReciveableAmount() {
		if(txtTotalReciveableAmount.getText().isEmpty()) {
			return "0";
		}
		return txtTotalReciveableAmount.getText().trim();
	}


	public void setTxtTotalReciveableAmount(String txtTotalReciveableAmount) {
		this.txtTotalReciveableAmount.setText(txtTotalReciveableAmount);
	}

	public void setTxtTotalReciveableAmount(double txtTotalReciveableAmount) {
		this.txtTotalReciveableAmount.setText(df.format(txtTotalReciveableAmount));
	}
	...
	public boolean getCheckInvoiceRecived() {
		return checkInvoiceRecived.isSelected();
	}

	public void setCheckInvoiceRecived(boolean checkInvoiceRecived) {
		this.checkInvoiceRecived.setSelected(checkInvoiceRecived);
	}
